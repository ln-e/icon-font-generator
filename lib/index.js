...
async function generate(options = {}) {
  ...
  // Log start
  let msg = `Generating font kit from ${options.paths.length} SVG icons`;
  log(options, msg);
...
function parseSelector(selector = '') {
  ...
  return {
    ...
    classNames: classNamesMatch
      ? classNamesMatch.map((cname) => cname.substr(1))
      : [],
  };
}
...
async function generateJson(options, generatorResult) {
  ...
  if (options.ligature) {
    const svg = generatorResult.svg;
    map.ligatures = [];

    svg.replace(SVG_PARSE_REGEX, (match, name, code) => {
      return map.ligatures.push(name);
    });
  } else {
    ...
    css.replace(
      CSS_PARSE_REGEX,
      (match, name, code) => (map[name] = parseInt(code.replace('\\', ''), 16)),
    );
  }
...
async function validateOptions(options) {
  ...
  if (options.codepoints) {
    if (!(await fsAsync.exists(options.codepoints))) {
      throw new ValidationError(
        `Cannot find json file @ ${options.codepoints}!`,
      );
    }

    const codepointsStat = await fsAsync.stat(options.codepoints);
    if (
      !codepointsStat.isFile() ||
      path.extname(options.codepoints) !== '.json'
    ) {
      throw new ValidationError(
        [

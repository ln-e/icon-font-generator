...
const {
        CSS_PARSE_REGEX, SVG_PARSE_REGEX, FONT_TYPES, TEMPLATES, OPTIONAL_PARAMS, DEFAULT_OPTIONS
      } = require('./constants')
...
async function generateJson(options, generatorResult) {
  ...
  if(options.ligature) {
    ...
    css.replace(CSS_PARSE_REGEX, (match, name, code) => map[name] = parseInt(code.replace('\\', ''), 16))
  }
